...
// const SLIDER_CONFIGS: SliderConfig[] = [
//   { key: "u_speed", label: "Movement Speed", min: 0, max: 2, step: 0.01 },
//   { key: "u_scale", label: "Pattern Size", min: 0.1, max: 2, step: 0.01 },
//   { key: "u_turbAmp", label: "Turbulence", min: 0, max: 2, step: 0.01 },
//   { key: "u_turbFreq", label: "Swirl Density", min: 0.01, max: 1, step: 0.01 },
//   { key: "u_turbIter", label: "Color Complexity", min: 2, max: 12, step: 1 },
//   { key: "u_waveFreq", label: "Wave Ripple", min: 0.5, max: 10, step: 0.1 },
//   { key: "u_distBias", label: "Flow Distortion", min: -2, max: 2, step: 0.01 },
//   { key: "u_ditherMode", label: "Noise Type", min: 0, max: 2, step: 1 },
//   { key: "u_dither", label: "Grain Amount", min: 0, max: 0.2, step: 0.005 },
//   { key: "u_exposure", label: "Brightness", min: 0, max: 3, step: 0.05 },
//   { key: "u_contrast", label: "Contrast", min: 0.5, max: 3, step: 0.05 },
//   { key: "u_saturation", label: "Vibrancy", min: 0, max: 2, step: 0.05 },
//   { key: "u_seed", label: "Random Style", min: 0, max: 1000, step: 1 },
//   { key: "u_jellify", label: "Jelly Effect", min: 0, max: 1, step: 1 },
// ];
...
//         {/* Giảm bớt opacity đen xuống 10% cho shader hiện lên rõ nét, không bị tối đen như ảnh cũ */}
//       </motion.div>

//       {/* Phần Debug Panel: Ép z-30 phóng khoáng để nó đè lên cả chữ (z-10) lẫn canvas (z-0) */}
//       <div
//         data-panel
//         style={panelStyle}
//         className={cn(
//           "dark z-30 w-67 pointer-events-auto", // <── Đảm bảo có z-30 và pointer-events-auto ở đây
//           "bg-card/95 backdrop-blur-lg border border-border rounded-lg",
//           "font-mono text-xs text-foreground select-none shadow-xl",
//           !panelPos && "fixed top-24 right-5",
//         )}
//       >
//         {/* Header */}
//         <div
...
//                         <label
//                           className={cn(
//                             "flex items-center gap-1.5 relative bg-muted/40 border border-border rounded-md px-2 py-1.25 cursor-pointer hover:bg-muted/60 transition-colors",
//                           )}
//                         >
//                           <div
//                             className="w-4 h-4 rounded shrink-0 border border-white/10"
  ...
  return (
    <>
      ...
      <div
        ...
        style={panelStyle}
        className={cn(
          "dark z-30 w-67 pointer-events-auto",
          "bg-card/95 backdrop-blur-lg border border-border rounded-lg",
          "font-mono text-xs text-foreground select-none shadow-xl",
          !panelPos && "fixed top-24 right-5",
        )}
...
//   u_turbAmp: 0.6,
//   u_turbFreq: 0.1,
//   u_turbIter: 7, // 🚀 Giữ nguyên 7 vòng lặp để form dáng uốn lượn sắc nét nhất
//   u_waveFreq: 3.8,
//   u_distBias: 0,
//   u_jellify: 0,
//   u_ditherMode: 2,
...
//       // 🚀 CHIÊU ĐỘC: Giảm dpr xuống hẳn 0.55 để nén khung đệm tính toán pixel đồ họa.
//       // GPU giảm tải tới 4 lần, chạy 240 FPS nhẹ tênh, chất lượng dải màu nhòe không bị ảnh hưởng.
//       const dpr = 0.55;
//       const renderW = Math.floor(w * dpr);
//       const renderH = Math.floor(h * dpr);
...
//     // 🚀 BUNG LỤA TỐI ĐA FPS: Chạy native 240Hz không chặn nhịp nhả frame hình
//     const render = (): void => {
//       if (!glRef.current) return;
//       if (!isIntersectingRef.current) {
//         animFrameRef.current = requestAnimationFrame(render);
...
//         className="absolute inset-0 z-0 w-full h-full overflow-hidden bg-black pointer-events-none"
//       >
//         {/* Thuật toán gl.LINEAR nội suy của GPU tự động làm mượt pixel khi upscale từ dpr 0.55 */}
//         <canvas
//           ref={canvasRef}
//           className="absolute inset-0 w-full h-full block object-cover"
//         />
