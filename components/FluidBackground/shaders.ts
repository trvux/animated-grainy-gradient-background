...
//     uvec3 s = uvec3(
//         floatBitsToUint(seedVal), floatBitsToUint(seedVal * 1.5 + 7.31), floatBitsToUint(seedVal * 2.7 + 13.37)
//     );
//     s = hash3(s);
...
//         mix(lch0.x, lch1.x, t), mix(lch0.y, lch1.y, t), lch0.z + dh * t
//     ));
// }
...
//     if (mode < 1.5) return IGN(I);       // 1: Smooth IGN
...
//     // Match reference: dampen time by 0.3 so speed uniform behaves correctly
//     float t = u_time * 0.3;
...
//             q.x += u_turbAmp * sin(q.y / freq * fj + t1 + a + seedOffset.x * fj) / fj;
//             q.y += (u_turbAmp * 0.1) * cos(q.x / freq * fj + seedOffset.y * fj) / fj; // Triệt tiêu t1 ở trục Y để chống xoắn vòng
